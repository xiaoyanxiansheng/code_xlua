...
#pragma warning restore 414
    ...
    public partial class ObjectTranslator
	{
        ...
        public bool TryDelayWrapLoader(RealStatePtr L, Type type)
        {
            ...
            LuaAPI.luaL_newmetatable(L, type.FullName); //先建一个metatable，因为加载过程可能会需要用到 存放在Reg中
            LuaAPI.lua_pop(L, 1);
            ...
            else
            {
                ...
                {
                    // 这里由于没有生成代码，所以需要使用反射的方式，编辑器下面就是这种逻辑
                    // UI相关并没有生成代码，都会走这里
        ...
        public ObjectTranslator(LuaEnv luaenv,RealStatePtr L)
		{
            ...
            LuaAPI.lua_newtable(L);                                     // stack : newTable1
            LuaAPI.lua_newtable(L);                                     // stack : newTable1 newTable2
            LuaAPI.xlua_pushasciistring(L, "__mode");                   // stack : newTable1 newTable2 "__mode"
            LuaAPI.xlua_pushasciistring(L, "v");                        // stack : newTable1 newTable2 "__mode" "v"
            LuaAPI.lua_rawset(L, -3);                                   // stack : newTable1 newTable2  : newTable2.__mode = "v"
            LuaAPI.lua_setmetatable(L, -2);                             // stack : newTable1            : newTable1.mt = newTable2
            cacheRef = LuaAPI.luaL_ref(L, LuaIndexes.LUA_REGISTRYINDEX);// stack :                      : reg[cacheRef] = newTable1
        ...
        internal int getTypeId(RealStatePtr L, Type type, out bool is_first, LOGLEVEL log_level = LOGLEVEL.WARN)
        {
            ...
            if (!typeIdMap.TryGetValue(type, out type_id)) // no reference
            {
                ...
                else
                {
                    ...
                    LuaAPI.lua_pushvalue(L, -1);                                    // stack : mt mt
                    type_id = LuaAPI.luaL_ref(L, LuaIndexes.LUA_REGISTRYINDEX);     // stack : mt
                    LuaAPI.lua_pushnumber(L, type_id);                              // stack : mt type_id
                    LuaAPI.xlua_rawseti(L, -2, 1);                                  // stack : mt          : mt[1] = type_id
        ...
        /// <summary>
        /// 根据userdata信息获取存在c#这边的对象
        /// </summary>
        /// <param name="L"></param>
        /// <param name="index">lua栈中的位置</param>
        /// <param name="udata">lua栈中的数据</param>
        /// <returns></returns>
