    ...
    public partial class LuaTable : LuaBase
    {
        ...
        public void Set<TKey, TValue>(TKey key, TValue value)
        {
            ...
            lock (luaEnv.luaEnvLock)
            {
                ...
                LuaAPI.lua_getref(L, luaReference);             // stack : table
                translator.PushByType(L, key);                  // stack : table key
                translator.PushByType(L, value);                // stack : table key value

                if (0 != LuaAPI.xlua_psettable(L, -3))          // stack : table            : table.key = value
                {
