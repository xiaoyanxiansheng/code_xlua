...
namespace XLua
{
    ...
    public partial class StaticLuaCallbacks
    {
        ...
        /*
         * 直接通过字符串设置或得到mt（eg：xlua.setmetatable(CS.XLuaTest.PushAsTableStruct, mt)）
         * set : reg[type_id] = mt
         * get : rt mt
         */
        [MonoPInvokeCallback(typeof(LuaCSFunction))]
        public static int XLuaMetatableOperation(RealStatePtr L)
        {
            try
            {
                int typeid = LuaDLL.Lua.xlua_tointeger(L,1);
                ObjectTranslator translator = ObjectTranslatorPool.Instance.Find(L);
                Type type = getType(L, translator, 1);
                if (type == null)
                {
                ...
                else if (param_num == 2) //set
                {
                    ...
                    LuaAPI.lua_pushnumber(L, type_id);  // stack : type_id metatable type_id
                    LuaAPI.xlua_rawseti(L, 2, 1);       // stack : type_id metatable                    : metatable[1] = type_id
                    LuaAPI.xlua_rawseti(L, LuaIndexes.LUA_REGISTRYINDEX, type_id);// stack : type_id    : reg[type_id] = metatable
                    return 0;
                }
                else
                {
