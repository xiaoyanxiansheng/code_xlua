...
// key->keyuserdata keyuserdata.mt = meta_ref
// 实际上就是将int索引转成一个userdata然后绑定meta使其拥有操作
...
//upvalue --- [1]:getters, [2]:feilds, [3]:base, [4]:indexfuncs, [5]:baseindex 如果没有就是站位nil
//param   --- [1]: obj, [2]: key
LUA_API int cls_indexer(lua_State *L) {	
	// 检测是否有getter 直接触发函数调用 和feilds有区别
	if (!lua_isnil(L, lua_upvalueindex(1))) {
		// push key
		lua_pushvalue(L, 2);
		// key -> getter[key]
	...
	// 检测是否有feilds 需要返回函数指针 由触发者去调用 和 getter不一样
	if (!lua_isnil(L, lua_upvalueindex(2))) {
		lua_pushvalue(L, 2);
	...
	// TODO 这个方法是一个Lazy的模式，如果需要在父类查找对应的字段和属性
	if (!lua_isnil(L, lua_upvalueindex(3))) {
		lua_pushvalue(L, lua_upvalueindex(3));
		while(!lua_isnil(L, -1)) {
			lua_pushvalue(L, -1);
